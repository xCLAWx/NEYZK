...
if MsgText[1] == 'اصدار الTHTSS' or MsgText[1] == 'الاصدار' then
return '👨🏾‍🔧¦ اصدار THTSS الزعيم : *v'..version..'* \n📡'
end
 
if (MsgText[1] == 'تحديث الTHTSS' or MsgText[1] == 'تحديث الTHTSS ™') then
if not msg.SudoBase then return "📛*¦* هذا الامر يخص {المطور الاساسي} فقط  \n🚶" end
...
sendMsg(arg.chat_id_,arg.id_,'📛*¦* عذرا اسم الملف غير مدعوم للنظام او لا يتوافق مع THTSS الزعيم يرجى جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n❕')
end  
else
...
if MsgText[1]== "قناة الTHTSS 📡" and msg.type == "pv" then
local inline = {{{text="قنآهہ‏‏ آلسـورس : الـزعـيـم 🍃",url="t.me/th3bs"}}}
send_key(msg.sender_user_id_,'  [قناة الTHTSS : الـزعـيـم](t.me/th3bs)',nil,inline,msg.id_)
return false
end
...
🔅¦ تحديث الTHTSS: لتحديث الTHTSS الى اصدار احدث
🔅¦ـ➖➖➖➖➖
...
if MsgText[1] == "THTSS" or MsgText[1]=="الTHTSS" then
return [[
        🛠 ┇ تنصـيب سـورس آلنيزك🔎
...
return "📛| اوبس , هناك خطأ في مصفوفه الجيسون راسل مطور الTHTSS ليتمكن من حل المشكله في اسرع وقت ممكن.!"
end
else
...
local keyboard = {
 ...
 {"تحديث ♻️","قائمه العام 📜","قناة الTHTSS 📡"},
 {"تحديث الTHTSS ™"},
 {"الغاء الامر ✖️"}}
